# 🐧 100 Ejercicios de Comandos Linux
...
- [📁 Listado y navegación de archivos](#-listado-y-navegación-de-archivos) (1–9)
- [📂 Gestión de directorios y archivos](#-gestión-de-directorios-y-archivos) (10–30)
- [🔗 Enlaces (hard y simbólicos)](#-enlaces-hard-y-simbólicos) (31–40)
- [🔐 Permisos](#-permisos) (41–56)
- [🔍 Búsqueda de archivos](#-búsqueda-de-archivos) (57–71)
- [↔️ Redirección y tuberías](#️-redirección-y-tuberías) (72–80)
- [⚙️ Procesos](#️-procesos) (81–86)
- [🛠️ Comandos de sistema](#️-comandos-de-sistema) (87–100)
...
18. Copiar el archivo `/etc/motd` a un archivo llamado `mensaje` en vuestro directorio `PRUEBA`.
...
33. Utilizando el enlace creado, copiar los archivos que empiecen por `u` del directorio `/bin` en el directorio `nuevo1`.
34. Crear dos enlaces duros del fichero `fich1`, llamados `enlace`, en los directorios `dir1` y `dir2`.
...
50. Establecer mediante el comando `umask` los siguientes valores por omisión: `rwxr--r--` para los directorios y `rw-r--r--` para los archivos ordinarios.
...
55. Crear en el directorio propio:
    ...
    - El directorio `carpeta2` con todos los permisos para el propietario y lectura y ejecución para los del mismo grupo. Dentro, `file1` con lectura y escritura para el propietario y los del grupo, y `file2` con los mismos permisos para el propietario y solo lectura para el grupo.
...
68. Visualizar de qué tipo son todos y cada uno de los ficheros de todo el árbol del sistema propiedad de un usuario conocido.
...
72. Visualizar con la orden `who` la relación de usuarios conectados y sus terminales. Mediante la orden `cat`, crear un pequeño mensaje desde tu consola y redirigirlo a uno de los terminales conectados.
73. Crear un archivo de tamaño 0.
74. Visualizar el archivo `/etc/motd`, que contiene el "mensaje del día".
...
80. Sacar por pantalla únicamente el tiempo que tarda en ejecutarse el comando `who` (buscar comando `time`).

---

## ⚙️ Procesos

81. Sacar por pantalla un listado completo de los procesos que está realizando el usuario `root` (buscar comando `ps`).
...
85. Averiguar cuál es la actividad actual del sistema. Para ello, visualizar un listado completo del estado de todos los procesos que se están ejecutando.
86. Obtener un listado con los datos de los procesos de tu shell actual.

---

## 🛠️ Comandos de sistema

87. Mostrar cuántos usuarios tiene registrados el sistema (el registro de usuarios está en el archivo `/etc/passwd`).
...
90. Mostrar las líneas de un archivo de texto que empiecen por `L` (mayúscula o minúscula).
...
> 💡 **Consejo:** Puedes consultar el manual de cualquier comando con `man <comando>` o buscar ayuda rápida con `<comando> --help`.
